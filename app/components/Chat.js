...
const Chat = ({ roomId = 'defaultRoomId' }) => {
  ...
  const handleSendVoiceMessage = async (audioUrl) => {
    if (user && roomId) {
      const username = user.email.split('@')[0];
      try {
        await addDoc(collection(db, 'rooms', roomId, 'messages'), {
      ...
      console.log('User or roomId is missing');
    }
  };
            ...
            <div className="flex-grow overflow-y-auto mb-4">
              {messages.map((message) => (
                <div key={message.id} className="mb-2 p-2 break-words">
                  ...
                  {message.audioUrl && (
                    <CustomAudioPlayer url={message.audioUrl} /> // Use custom audio player
                  )}
                </div>
              ))}
            </div>
        ...
        {isOpen ? (
          ...
          >
            ...
            <div className="flex items-center relative">
              ...
              <VoiceRecorder onSend={handleSendVoiceMessage} className="ml-2" /> {/* Align microphone button */}
            </div>
          </ResizableBox>
        ) : (
          <button
